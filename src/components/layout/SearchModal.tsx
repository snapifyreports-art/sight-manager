...
export function SearchModal({
  ...
  const groups = results ? [
    ...
    { key: "contacts", label: "Contacts", icon: Users, color: "text-orange-600", items: results.contacts.map((c) => ({ id: c.id, label: c.company ? `${c.company} — ${c.name}` : c.name, sub: c.type === "CONTRACTOR" ? "Contractor" : "Supplier", href: `/contacts/${c.id}` })) },
